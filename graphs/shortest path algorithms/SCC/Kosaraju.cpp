...
Perform DFS (dfsFill) on the original graph to push vertices into a stack in order of finishing time.

Build the reversed graph by reversing all edges.

Pop nodes from the stack and run DFS (dfsTraverse) on the reversed graph.

Each DFS run gives one SCC → increment count and finally return i
